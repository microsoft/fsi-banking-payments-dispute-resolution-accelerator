...
### Public repo prep — resource identifier redaction and history squash (2026-08-14)

- **Resource-name token sweep:** The AZD-generated resource-name token embedded in all deployed Azure resource names was found across 16 files at HEAD. Every resource name containing the token was replaced with a clearly-named placeholder (`<STORAGE_ACCOUNT_NAME>`, `<COSMOS_ACCOUNT_NAME>`, `<FUNCTION_APP_NAME>`, `<AI_SERVICES_NAME>`, `<EVENT_GRID_TOPIC_NAME>`, `<ANALYST_SWA_NAME>`, `<PORTAL_SWA_NAME>`). FQDNs were handled separately (e.g., `.blob.core.windows.net` suffix preserved; `.documents.azure.com` suffix preserved).
- **Subscription/tenant name sweep:** The managed subscription name and tenant domain appearing in docs, squad records, and source-file comments were replaced with `<AZURE_SUBSCRIPTION_NAME>` and `<AZURE_TENANT_DOMAIN>` respectively. A bare subscription qualifier appearing in prose was rewritten to "the managed Azure subscription" or "Managed Azure subscription tenants" for grammatical flow.
- **Live SWA hostnames:** Two live Static Web App hostnames referenced in `docs/architecture.md`, `README.md`, `src/web/playwright.config.ts`, and agent history were replaced with `<ANALYST_SWA_HOSTNAME>` and `<PORTAL_SWA_HOSTNAME>`. Mermaid diagram nodes updated in-place; table rows updated in-place. Diagram remains renderable.
- **Source file treatment:** `DEFAULT_ENDPOINT` constants in `src/api/services/embeddings_client.py` and `evidence_search_agent_client.py` were updated in-place (placeholder URL replacing live endpoint). No structural refactoring — the existing env-var fallback mechanism and function signatures are unchanged, per explicit user instruction. A brief comment was added to each constant noting the env var override path.
- **Verification:** All eleven search patterns (`git grep -i -E`) returned zero hits outside seed/synthetic/test directories after redaction. Test suite: 403 passed, 0 failed.
